...
1. [Parser Configuration](references/_sections.md#1-parser-configuration) — **CRITICAL**
   ...
   - 1.8 [Validate JSON Parser Input](references/parser-json-validation.md) — CRITICAL (prevents runtime crashes and unsafe casts from URL-supplied JSON)
...
3. [State Management](references/_sections.md#3-state-management) — **HIGH**
   ...
   - 3.5 [Use Setter Return Value for URL Access](references/state-setter-return.md) — MEDIUM (enables accurate URL tracking for analytics/sharing without re-deriving the URL)
   - 3.6 [Use Standard Schema for Cross-Library Validation](references/state-standard-schema.md) — MEDIUM (eliminates 2-4× duplicate schema definitions across nuqs, tRPC, route validators, and forms)
   - 3.7 [Use useQueryStates for Related Parameters](references/state-use-query-states.md) — HIGH (gives a single typed object and one combined URLSearchParams flush)
   - 3.8 [Use withOptions for Parser-Level Configuration](references/state-options-inheritance.md) — MEDIUM (reduces boilerplate and ensures consistent behavior)
4. [Server Integration](references/_sections.md#4-server-integration) — **HIGH**
   ...
   - 4.5 [Use createSearchParamsCache (or createLoader) for Server Components](references/server-search-params-cache.md) — HIGH (eliminates prop drilling across N component levels)
...
8. [Advanced Patterns](references/_sections.md#8-advanced-patterns) — **LOW**
   ...
   - 8.5 [Use urlKeys for Shorter URLs](references/advanced-url-keys.md) — LOW (reduces URL length by 50-70% for verbose params)
...
3. [https://github.com/47ng/nuqs](https://github.com/47ng/nuqs)
4. [https://nextjs.org/docs](https://nextjs.org/docs)
5. [https://react.dev](https://react.dev)
6. [https://standardschema.dev](https://standardschema.dev)
